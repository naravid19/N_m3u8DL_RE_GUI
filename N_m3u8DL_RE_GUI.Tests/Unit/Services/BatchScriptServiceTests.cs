...
public class BatchScriptServiceTests
{
    ...
    [Fact]
    public async Task BuildScriptAsync_WithNullOrEmptyInputs_ShouldThrowArgumentException()
    {
        var service = new BatchScriptService();

        await Assert.ThrowsAsync<ArgumentException>("inputPath", () =>
            service.BuildScriptAsync("", "exe.exe", _ => Task.FromResult("t"), _ => ""));

        await Assert.ThrowsAsync<ArgumentException>("exePath", () =>
            service.BuildScriptAsync("valid.txt", "", _ => Task.FromResult("t"), _ => ""));
    }
    ...
    public async Task BuildScriptAsync_WithDirectoryInput_ShouldIncludeOnlyM3u8AndMpd()
    {
        ...
        try
        {
            ...
            Assert.Contains("batch_", Path.GetFileName(result.FilePath));
            Assert.EndsWith(".bat", result.FilePath);
            Assert.Contains("@echo off", lines);
            Assert.Contains("chcp 65001 >nul", lines);
    ...
    [Fact]
    public async Task BuildScriptAsync_ShouldGenerateUniqueFilePathPerCall()
    {
        var service = new BatchScriptService();
        var tempDir = Directory.CreateTempSubdirectory("batch-unique");
        var txtPath = Path.Combine(tempDir.FullName, "list.txt");
        File.WriteAllLines(txtPath, new[] { "https://example.com/ep1.m3u8" });

        try
        {
            var result1 = await service.BuildScriptAsync(txtPath, @"C:\exe.exe", _ => Task.FromResult("t"), _ => "");
            var result2 = await service.BuildScriptAsync(txtPath, @"C:\exe.exe", _ => Task.FromResult("t"), _ => "");
    ...
    [Fact]
    public async Task BuildScriptAsync_WithTxtInput_ShouldUseParsedItemIndexDenominator()
    {
        var service = new BatchScriptService();
        ...
        try
        {
            ...
            Assert.Contains("TITLE \"[1/2] - Episode 1\"", result.Content);
            Assert.Contains("TITLE \"[2/2] - Auto-Title\"", result.Content);
    ...
    [Fact]
    public async Task BuildScriptAsync_WhenTitleResolutionFails_ShouldFallbackToUrl()
    {
        var service = new BatchScriptService();
        var tempDir = Directory.CreateTempSubdirectory("batch-fallback");
        var txtPath = Path.Combine(tempDir.FullName, "list.txt");
        const string targetUrl = "https://example.com/failed-stream.m3u8";
        File.WriteAllLines(txtPath, new[] { targetUrl });
        ...
        try
        {
            var result = await service.BuildScriptAsync(
                inputPath: txtPath,
                exePath: @"C:\Tools\N_m3u8DL-RE.exe",
                resolveTitleAsync: _ => throw new InvalidOperationException("HTTP Timeout"),
                buildArgsForInput: _ => "--arg");

            Assert.Contains(targetUrl, result.Content);
        }
        finally
        {
            tempDir.Delete(recursive: true);
